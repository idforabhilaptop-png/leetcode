...
const UpdatePanel = () => {
        ...
        const fetchProblem = async () => {
            try {
                ...
                reset({
                    ...
                    invisibleTestCases: problem.invisibleTestCases,
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            navigate('/admin_panel/update&delete');
        } catch (error) {
            console.error(error.response?.data || error.message);
            const errorMsg = error.response?.data || error.message;
            alert(`Failed to update problem: ${errorMsg}`);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-indigo-200">
                            <Pencil size={28} strokeWidth={3} />
                        </div>
                        <h1 className="text-3xl lg:text-4xl font-extrabold text-slate-900 tracking-tight">Update Challenge</h1>
                    </div>
                    <p className="text-slate-500 font-medium">Modify and improve existing platform challenges.</p>
                </div>

                <div className="flex items-center gap-4">
                    <button
                        type="button"
                        onClick={() => navigate('/admin_panel/update&delete')}
